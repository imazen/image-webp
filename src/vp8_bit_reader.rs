...
impl<'a> PartitionReader<'a> {
    ...
    /// Get state for manual coefficient decoding (shadowing optimization).
    /// Returns (value, range, bits, data_ptr, pos, data_len, eof).
    #[inline(always)]
    pub fn get_state_for_coeffs(&self) -> (u64, u32, i32, *const u8, usize, usize, bool) {
        (
            self.state.value,
            self.state.range,
            self.state.bits,
    ...
    /// Commit state after manual coefficient decoding.
    #[inline(always)]
    pub fn commit_state(&mut self, value: u64, range: u32, bits: i32, pos: usize, eof: bool) {
...
/// Fast coefficient decoding with shadowed state.
/// This module provides optimized coefficient reading that shadows the bit reader
/// state in local variables to avoid memory stores on every bit read.
pub mod fast_coeffs {
    use super::BITS;

    /// Load bytes into the value buffer.
    /// Returns (new_value, new_bits, new_pos, hit_eof).
    #[inline(always)]
    pub unsafe fn load_new_bytes_fast(
        value: u64,
        bits: i32,
        data: *const u8,
        pos: usize,
        ...
        if remaining >= 7 {
            // Fast path: read 7 bytes at once
            #[cfg(target_pointer_width = "64")]
            {
                if remaining >= 8 {
                    let ptr = data.add(pos);
                    let in_bits_full = u64::from_be_bytes(core::ptr::read_unaligned(ptr as *const [u8; 8]));
                    let new_bits = (in_bits_full >> 8) | (value << BITS);
                    return (new_bits, bits + BITS, pos + 7, false);
                }
            }
            // Fallback for exactly 7 bytes
            let mut in_bits: u64 = 0;
            for i in 0..7 {
                in_bits = (in_bits << 8) | u64::from(*data.add(pos + i));
            }
            (in_bits | (value << BITS), bits + BITS, pos + 7, false)
        } else if remaining > 0 {
            // Slow path: read one byte at a time
            let byte = *data.add(pos);
            (u64::from(byte) | (value << 8), bits + 8, pos + 1, false)
        } else if !already_eof {
            // First time hitting EOF - pad with zeros
            (value << 8, bits + 8, pos, true)
        } else {
            // Already hit EOF - stop incrementing bits
            (value, 0, pos, true)
        }
    }

    /// Read a single bit with given probability.
    /// Returns (bit, new_value, new_range, new_bits).
    #[inline(always)]
    pub unsafe fn get_bit_fast(
        prob: u8,
        value: u64,
        range: u32,
        bits: i32,
        ...
        // Load bytes if needed
        if bits < 0 {
            let (new_value, new_bits, new_pos, hit_eof) =
                load_new_bytes_fast(value, bits, data, *pos, len, *eof);
            value = new_value;
            bits = new_bits;
            *pos = new_pos;
            *eof = hit_eof;
        ...
        let split = (range.wrapping_mul(u32::from(prob))) >> 8;
        let val_shifted = (value >> bits) as u32;
        let bit = if val_shifted > split { 1 } else { 0 };

        let mut new_range = if bit != 0 {
            value = value.wrapping_sub((u64::from(split) + 1) << bits);
            range - split
        } else {
            split + 1
        };

        let shift = 7 ^ (31 ^ new_range.leading_zeros() as i32);
        new_range <<= shift;
        let new_bits = bits - shift;
        new_range = new_range.wrapping_sub(1);

        (bit, value, new_range, new_bits)
    }

    /// Read the sign bit and apply it to the value.
    /// Returns the signed value.
    #[inline(always)]
    pub unsafe fn get_signed_fast(
        v: i32,
        value: &mut u64,
        range: &mut u32,
        bits: &mut i32,
        ...
        // Save original bits position before decrementing
        let bit_pos = *bits;
        let split = *range >> 1;
        let val_shifted = (*value >> bit_pos) as u32;
        let mask = (split.wrapping_sub(val_shifted) as i32) >> 31;

        *bits = bit_pos - 1;
        *range = range.wrapping_add(mask as u32);
        *range |= 1;

        let term = (u64::from(split) + 1) & (mask as u32 as u64);
        *value = value.wrapping_sub(term << bit_pos);
...
mod tests {
    ...
    /// Test fast_coeffs functions against regular PartitionReader methods
    #[test]
    fn test_fast_coeffs_vs_partition_reader() {
        use super::fast_coeffs;

        let data: Vec<u8> = (0..512).map(|i| (i * 17 + 3) as u8).collect();
        let boundaries = [(0, data.len())];

        // Setup two identical partitions
        let mut parts1 = VP8Partitions::new();
        parts1.init(data.clone(), &boundaries);

        let mut parts2 = VP8Partitions::new();
        parts2.init(data.clone(), &boundaries);

        let probs = [128, 64, 192, 100, 200, 32, 224, 80];

        // Test get_bit
        {
            let mut reader1 = parts1.reader(0);
            let (mut value, mut range, mut bits, data_ptr, mut pos, len, mut eof) =
                parts2.reader(0).get_state_for_coeffs();
            drop(parts2.reader(0));

            let mut diffs = 0;
            for &prob in &probs {
                for _ in 0..20 {
                    let bit1 = reader1.get_bit(prob);
                    let (bit2, nv, nr, nb) = unsafe {
                        fast_coeffs::get_bit_fast(prob, value, range, bits, data_ptr, &mut pos, len, &mut eof)
                    };
                    value = nv;
                    range = nr;
                    bits = nb;
                    ...
                    if bit1 != bit2 {
                        ...
                        if diffs <= 5 {
                            eprintln!(
                                "get_bit diff: prob={}, regular={}, fast={}",
                                prob, bit1, bit2
                            );
                        }
                    }
            ...
            assert_eq!(diffs, 0, "get_bit_fast should match get_bit");
        }
    }
}
